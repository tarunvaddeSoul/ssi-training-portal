...
export default function PHCPage() {
  ...
  const verifyPHC = async () => {
    try {
      const response = await axios.post('http://localhost:3000/agent/verify-phc');
  ...
  const checkVerificationState = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/agent/verification-state/id/${verificationId}`);
      const data = response.data;
  ...
  const issuePHC = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:3000/agent/issue-phc/name/${encodedName}`);
      const data = response.data;
      if (data.statusCode === 201) {
  ...
  const checkIssuanceState = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/agent/credential-state/id/${verificationId}`);
      const data = response.data;
      if (data.statusCode === 200) {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <Card>
        ...
        <CardContent>
          <p className="mb-4">A Personhood Credential (PHC) is a unique digital credential that verifies your identity as a real person without revealing personal information.</p>
          <Button onClick={() => setIsModalOpen(true)}>Get your PHC Now!!</Button>
        </CardContent>
      </Card>
          ...
          <div className="mt-4">
            ...
            {currentStep === 1 && (
              <div>
                ...
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${qrCode}`} alt="Verification QR Code" className="mx-auto mt-4" />
              </div>
            )}
            {currentStep === 2 && (
              <div>
                <Input
                  ...
                  className="mb-4"
                />
                <Button onClick={issuePHC} disabled={!name}>Issue New PHC</Button>
              </div>
            )}
            {currentStep === 3 && (
              <div>
                <p>Scan this QR code with your wallet to receive your PHC:</p>
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${qrCode}`} alt="Issuance QR Code" className="mx-auto mt-4" />
              </div>
            )}
            {currentStep === 4 && (
              <Alert>
                <AlertTitle>Success!</AlertTitle>
                <AlertDescription>Your PHC has been issued and added to your wallet.</AlertDescription>
              </Alert>
            )}
            {errorMessage && (
